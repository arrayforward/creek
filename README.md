...
| 📊 **可观测性** | OTLP + OpenMetrics | HTTP OTLP Exporter 推送 + Pull 拉取 + 读取清零 |
| 🌐 **多协议** | gRPC + JSON-RPC | Server Reflection + HTTP POST `/rpc`，Header 传递路由参数 |
| 🧩 **沙箱控制面** | WASM 过滤器 | wasm3 运行时 + JSON 配置管道 (delay/mirror/canary/reject) |
| 🧪 **测试** | 五级自动化 | 79 项协议 + 92 路由 + 13 JSON-RPC + 18 WASM + 双 E2E |
...
- **声明式路由规则** — 通过 Admin gRPC API 热更新 StickyBalancer 策略和 CircuitBreaker 阈值

### 中期 (v0.6)

- **S3 / Kafka Backend 注册** — Backend 可注册为非 gRPC 端点，Leaf 自动适配协议
- **多集群 Federation** — 多 DataCenter 跨集群路由，区域感知就近转发
- **WASM 自定义过滤器** — 用户编写 AssemblyScript/Rust 编译为 .wasm，Node 侧热加载执行

### 远期 (v1.0)

- **eBPF XDP 加速** — 热路径（UDP 分片/重组、FEC 解码）卸载到 eBPF XDP hook
- **QUIC 兼容模式** — Tight 协议可通过 QUIC 隧道穿透 NAT 和防火墙
- **控制面集群** — 多 Node 选举 Leader，全局一致的 StickyBalancer 状态和 CircuitBreaker 指标聚合
- **Prometheus Remote Write** — 直接写入 Prometheus 兼容时序数据库，替代 OTLP Collector 中间层
