...
| 🔄 **协议** | Tight UDP | 48 字节包头、CRC32、R-S 单冗余 FEC、Token-Bucket Pacer |
...
| 🌳 **高可用** | 多父 Leaf | Leaf 同时连接多个 Node，自动故障切换 |
| 🗂️ **目录同步** | LWW 版本合并 | 基于 Endpoint 版本号和时间戳的全网增量同步 |
| 🔍 **服务发现** | Redis + 静态 | 可选 Redis Hash 自动发现，兼容 `--peer` / `--parent` |
| 📊 **可观测性** | OTLP + OpenMetrics | gRPC OTLP Exporter 推送 + HTTP Pull 拉取 + 读取清零 |
| 🌐 **多协议** | gRPC + JSON-RPC | Leaf 暴露 gRPC Server Reflection + HTTP POST `/rpc` |
| ⚡ **QoS** | 三级优先级 | RPC 调用 normal/high/critical 分级，优先级队列调度 |
| 🧪 **测试** | 三级自动化 | 79 项协议单测 + 路由/JSON-RPC 单测 + gRPC/JSON-RPC 双 E2E |
...
# ✅ creek_e2e_grpc              — gRPC 多进程端到端
# ✅ creek_e2e_jsonrpc           — JSON-RPC 端到端
```
...
│   ├── runtime.cpp          # Node / Leaf 多父 / shard_key / SQL 路由
...
- **社区插件 API** — `RpcFilter` 抽象接口，支持自定义路由、鉴权、限流逻辑的热加载
- **W3C Trace Context 传播** — Tight 链路注入 traceparent/tracestate，全链路追踪
- **S3 / Kafka Backend 注册** — Backend 可注册为非 gRPC 端点，Leaf 自动适配协议

### 中期 (v0.5)

- **gRPC-JSON 转码桥** — Leaf 内置 HTTP/1.1 RESTful API → gRPC 自动转码
- **声明式路由规则** — 通过 Admin gRPC API 热更新 StickyBalancer 策略
- **自适应限流** — 基于 Leaf 本地指标（延迟/错误率）自动熔断故障后端

### 远期 (v1.0)

- **Cluster Federation** — 多 DataCenter 跨集群路由，区域感知就近转发
- **eBPF 加速** — 热路径（UDP 分片/重组、FEC 解码）卸载到 eBPF XDP hook
- **WASM 沙箱控制面** — Node 侧注入 WASM 模块实现流量镜像、故障注入和灰度发布
